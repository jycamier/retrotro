...
func (h *WebSocketHandler) handleTimerStart(client *ws.Client, payload json.RawMessage) {
	...
	h.timerService.StartTimer(context.Background(), retroID, data.DurationSeconds)
}
...
func (h *WebSocketHandler) handleTimerPause(client *ws.Client) {
	...
	h.timerService.PauseTimer(context.Background(), retroID)
}
...
func (h *WebSocketHandler) handleTimerResume(client *ws.Client) {
	...
	h.timerService.ResumeTimer(context.Background(), retroID)
}
...
func (h *WebSocketHandler) handleTimerAddTime(client *ws.Client, payload json.RawMessage) {
	...
	h.timerService.AddTime(context.Background(), retroID, data.Seconds)
}
	...
	if previousPhase == models.PhaseWaiting {
		...
		if err == nil {
			...
			for _, member := range teamMembers {
				h.attendeeRepo.Record(ctx, retroID, member.UserID, connectedUserIds[member.UserID])
			}
		}
	}
